...
#include <algorithm>
#include <sstream>
IllegalPick::IllegalPick(int x, int y): runtime_error("division by zero"), x(x), y(y) {}
const char* IllegalPick::what() const throw() {
    ostringstream stream = ostringstream("");
    stream << runtime_error::what() << "Position " << x << ", " << y << "is invalid." << endl;
    return stream.str().c_str();}
IllegalPlacement::IllegalPlacement(int x, int y) : runtime_error("division by zero"), x(x), y(y) {
}
...
Table::Table() : minRow(102), minCol(102), maxRow(0), maxCol(0) {
    ...
    shared_ptr<AnimalCard> startStack = shared_ptr<AnimalCard>(new StartStack());
...
Table& Table::operator+=(shared_ptr<NoSplit> card) {
    shared_ptr<StartStack> startStack = dynamic_pointer_cast<StartStack>(cardBoard[52][52]);
    *startStack += card;
    return *this;}
Table& Table::operator-=(shared_ptr<NoSplit> card) {
    shared_ptr<StartStack> startStack = dynamic_pointer_cast<StartStack>(cardBoard[52][52]);
